...
def main():
    ...
    k = 16 # Not change in this problem
    ...
    kSample = [] # a list of errors for 50 samples using k-neighbors
    for i in range(50):
        random.shuffle(data)
        testData = data[0:100]
        ...
        for eachDraw in testData:
            ...
            predictedRating_k = user_based_cf(priorData, userid, movieid, distance, k, i_flag, numOfUsers, numOfItems)[1]
            ...
            if (trueRating == 0) or (trueRating == predictedRating_k):
                error100_k.append(0)
            else:
                error100_k.append(1)
        ...
        errorAverage_k = sum(error100_k) / float(100)
        # Now you have average error for each sample
        # you need to save it into a list
        kSample.append(errorAverage_k)
        print i
        print k, 'Sample', kSample
